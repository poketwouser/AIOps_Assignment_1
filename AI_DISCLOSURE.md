...
Used mainly for **code generation** — the scripts and notebooks in this repository:
...
- **`Report.pdf` and the repository READMEs** — drafted with the same tool from my existing `Answer.pdf` write-ups and the contents of this repository, then reviewed and edited by me. The LaTeX formatting of `Report.pdf` was also generated.

AI was **not** used for the analysis itself. The technical-debt diagnosis in `q1_technical_debt/Answer.pdf` and the experiment analysis in `q2_mlflow_mnist/Answer.pdf` — the choice of debt categories, the reading of the results, and the argument that learning rate dominates batch size — are my own.
